...
__export(react_exports, {
  ...
  useBorrowerProfile: () => useBorrowerProfile,
  useCipherCredit: () => useCipherCredit,
  useCreditTier: () => useCreditTier,
  useLoanInfo: () => useLoanInfo,
  useMaxBorrowable: () => useMaxBorrowable,
  usePoolStats: () => usePoolStats,
  useRepaymentStats: () => useRepaymentStats,
...
function formatTimeLeft(dueDate) {
  ...
  const hours = Math.floor(msLeft % 864e5 / 36e5);
...
function usePoolStats() {
  const sdk = useCipherCredit();
  const fn = (0, import_react.useMemo)(() => sdk ? () => sdk.getPoolStats() : null, [sdk]);
  return useAsync(fn);
}
